...
#' @param cv_folds Either an origami fold object (as generated by
#'  \code{\link[origami]{make_folds}}) specifying a cross-validation scheme,
#'  or a \code{numeric} integer specifying the number of folds to be created
#'  for cross-validation. Use of cross-validation allows for entropy
#'  conditions on the one-step estimator to be relaxed. For compatibility with
#'  \code{\link[origami]{make_folds}}, the number of folds specified must be
#'  greater than or equal to 2; the default is to create 5 folds.
#'
#' @importFrom assertthat assert_that
...
est_onestep <- function(data,
  ...
  if (is.numeric(cv_folds)) {
    ...
    folds <- origami::make_folds(
      data, fold_fun = origami::folds_vfold, V = cv_folds
    )
  } else if (is.list(cv_folds) && class(cv_folds[[1]]) == "fold") {
    assertthat::assert_that(length(cv_folds) > 1L)
    folds <- cv_folds
  } else {
    stop("cv_folds must either be an origami fold object or an integer.")
  }
...
#'  \code{\link[origami]{make_folds}}, the number of foldsmust be greater than
#'  or equal to 2; the default is to create 5 folds.
